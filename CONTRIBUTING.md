...
repin` moves each `cs-` pin to the newer of its project's last CI build and its newest local one,
which a clean `make ci` records. It names each pin taken from a local build, and leaves one whose
project has neither. `make repin LOCAL=0` takes CI builds only. It moves the viewer's
`@codesweep-ai/ui` pin the same way, through `scripts/repin-npm.mjs`, and rebuilds the committed
viewer on it. `make versions` says which builds the gates used.
