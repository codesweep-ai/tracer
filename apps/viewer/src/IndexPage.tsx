...
function Lane({ trace, depth, hinted, parentId, spawnIndex }: { trace: LoadedTrace; depth: number; hinted: boolean; parentId?: string; spawnIndex?: number }) {
  ...
  const model = meta.model ?? "Unknown model", tokens = `${compact(totals.input + totals.output)} tokens`, time = timeLabel(totals.time), cost = traceCostLabel(totals);
  ...
  return <div ref={lane} data-testid="lane" data-trace-id={trace.id} data-parent-trace-id={parentId} data-spawn-index={spawnIndex} className="lane" style={{ marginLeft: `calc(${depth} * var(--trajectory-depth-indent))` }}>
    ...
    <Card variant="tight">
      <div className="lane-grid">
        <div className="lane-meta"><a href={linkTo(trace.id)} className="lane-title">{meta.title ?? meta.label ?? (meta.autoTitle ? <span className="auto-title" title="Derived from the session's first user message">{meta.autoTitle}</span> : trace.id)}</a><p className="lane-meta-line lane-meta-model" title={`${tokens} · ${model}`}><span className="lane-tokens">{tokens} ·&nbsp;</span><span className="lane-model"><bdi>{model}</bdi></span></p><p className="lane-meta-line" title={time}>{time}</p>{cost && <p className="lane-meta-line" title={cost}>{cost}</p>}{(meta.title ?? meta.label ?? meta.autoTitle) && <p className="lane-meta-line lane-id" title={trace.id}>{trace.id}</p>}{parentId && spawnIndex != null && hasTrace(parentId) && <p className="lane-meta-line"><a data-testid="fork-origin" href={linkTo(parentId, spawnIndex)} className="fork-origin">forked from #{spawnIndex}</a></p>}<div className="lane-badges">{totals.toolErrors > 0 && <StatusBadge label={`${totals.toolErrors} error`} status="error" />}{(parse.unreadable ?? 0) > 0 && <StatusBadge label={`${parse.unreadable} unreadable`} status="error" />}{parse.unrecognized > 0 && <StatusBadge label={`${parse.unrecognized} unrecognized`} status="warning" />}</div></div>
