...
The index lists each trajectory with its title, its model, its token count, its time and its cost.
The time reads as working time, then how long the trajectory was open, as in `4.4 h working · 211.4 h
open`. Working time is what the events account for, and the rest of the open time is mostly idle. The cost is the figure the CLI reported where it reported one, and otherwise tracer's estimate,
